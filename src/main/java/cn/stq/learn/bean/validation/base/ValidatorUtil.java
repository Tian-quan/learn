...
public final class ValidatorUtil {
	...
	private static boolean isExcept(String field, String... exceptFieldName){
		for(String obj : exceptFieldName){
			if(null!=obj && obj.trim().length()!=0 && field.indexOf(obj)>=0){
				return true;
			}
		}
		return false;
	 ...
	 */
	public static Map<String, String> validateGroupToMap(Object obj, Class<?>... groups){
		Map<String, String> addError = new HashMap<>();
		if(null == obj){
			throw new NullPointerException("被验证对象不能为null");
		}
		Set<ConstraintViolation<Object>> validateSet = validator.validate(obj, groups);
		for(ConstraintViolation<Object> constraintViolation : validateSet){
			addError.put(constraintViolation.getPropertyPath().toString(), constraintViolation.getMessage());
		}
		return addError;
	}
}